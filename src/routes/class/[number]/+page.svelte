...
<div class="space-y-6 py-2">
	<div>
		...
		{#await streamed.results}
			<div class="mt-1 h-4 w-16 skeleton"></div>
		{:then results}
			<p class="mt-1 text-sm text-surface-500">{results.totalCount} verse{results.totalCount !== 1 ? 's' : ''}</p>
		{/await}
	</div>
	...
	{:then results}
		{#if results.verses.length > 0}
			...
			{#if Math.ceil(results.totalCount / 20) > 1}
				<nav class="flex items-center justify-center gap-2 pt-4" aria-label="Pagination">
					...
					{/if}
					<span class="px-3 text-sm text-surface-500">{data.currentPage} / {Math.ceil(results.totalCount / 20)}</span>
					{#if data.currentPage < Math.ceil(results.totalCount / 20)}
						<a
							href="/class/{data.classNumber}?page={data.currentPage + 1}"
							class="flex items-center gap-1 rounded-xl border border-surface-200 bg-white px-4 py-2.5 text-sm font-medium text-surface-700 transition-all hover:bg-surface-50 active:scale-95   "
						>
